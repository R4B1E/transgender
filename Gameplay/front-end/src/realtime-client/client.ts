...
function Realtime(serverUrl: string, options?: { reconnect?: boolean; reconnectMaxDelayMs?: number; heartbeatIntervalMs?: number }) {
    ...
    function connect() {
        ...
        ws.addEventListener("close", (e) => {
            console.log(`connection closed at ${Date.now()} with code ${e.code} for the following reason : ${e.reason}`);
            // might break here
            stopHeartbeat();
    ...
    */
    async function subscribe(channel: Channel, handler: MsgHandler) {
    ...
    *  @param handler function handler for the event
    */
    async function publish(channel: Channel, data: PubData) {
        send({ event: "publish", channel, data });
    }
